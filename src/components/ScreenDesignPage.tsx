...
import {
  ArrowLeft, Maximize2, GripVertical, Layout, Smartphone, Tablet, Monitor,
  LayoutDashboard, BookOpen, PlusCircle, Users, Mail, BarChart3,
  GraduationCap, Settings, ShoppingBag, Ticket
} from 'lucide-react'
    ...
    return React.lazy(async () => {
      try {
        ...
        const ShellWrapper = ({ children }: { children?: React.ReactNode }) => {
          // Determine shell variant based on section ID
          const getShellVariant = (id?: string): 'public' | 'student' | 'educator' | 'admin' => {
            if (!id) return 'student'
            if (id.includes('educador') || id.includes('educator')) return 'educator'
            if (id.includes('admin')) return 'admin'
            if (id.includes('landing') || id.includes('catalogo') || id.includes('catalog')) return 'public'
            return 'student' // Default for student-panel, etc.
          }

          const variant = getShellVariant(sectionId)

          // Navigation items per variant (with icons)
          // "Preview" is always the first active item - generic for any screen being previewed
          const navigationByVariant = {
            educator: [
              { label: 'Preview', href: '/preview', icon: Layout, isActive: true },
              ...
              { label: 'Comunicaciones', href: '/comunicaciones', icon: Mail },
              { label: 'Estadísticas', href: '/estadisticas', icon: BarChart3 },
            ],
            student: [
              { label: 'Preview', href: '/preview', icon: Layout, isActive: true },
              { label: 'Mis Cursos', href: '/cursos', icon: BookOpen },
              { label: 'Órdenes', href: '/ordenes', icon: ShoppingBag },
            ],
            admin: [
              { label: 'Preview', href: '/preview', icon: Layout, isActive: true },
              { label: 'Dashboard', href: '/dashboard', icon: LayoutDashboard },
            ...
            public: [
              { label: 'Cursos', href: '/cursos', icon: BookOpen, isActive: true },
              { label: 'Sobre WSET', href: '/wset', icon: GraduationCap },
              { label: 'Contacto', href: '/contacto', icon: Mail },
            ],
          }

          const navigationItems = navigationByVariant[variant] || navigationByVariant.student

          const defaultUser = {
            name: 'Gabriela Zimmer',
            avatarUrl: '/gabi.webp',
            role: variant === 'public' ? undefined : variant,
          }
